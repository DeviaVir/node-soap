...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  for (i = 0; i < children.length; i++) {
    ...
    if (part.$element) {
      ...
      var element = schema.elements[nsName.name];
      element.targetNSAlias = ns;
      element.targetNamespace = definitions.xmlns[ns];
      // this.elements.push(element);
      this.elements[nsName.name] = element;

      // set the optional $lookupType to be used within `client#_invoke()` when
      // calling `wsdl#objectToDocumentXML()
      element.$lookupType = part.$name;
      ...
      if (lookupTypes.length > 0) {
        lookupTypes = lookupTypes.
            join('_').
            split('_').
            filter(isEmptyLookup);

        var schemaXmlns = definitions.schemas[element.targetNamespace].xmlns;
        ...
        if (typeNs) {
          if (type.name in Primitives) {
            ...
            if (ctype) {
              ...
              _.assign(this.parts, ctype.description(definitions, schema.xmlns));
            }
          }
        }
      }
      else {
        var method = element.description(definitions, schema.xmlns);
        this.parts = this.parts || {};
        _.assign(this.parts, method[nsName.name]);
      }
    } else {
      this.parts = this.parts || {};
...
OperationElement.prototype.postProcess = function(definitions, tag) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (message.elements) {
      for (var k in message.elements) {
        var element = message.elements[k];
        definitions.messages[element.$name] = message;
        this[child.name] = element;
      }
    }
    else {
...
MessageElement.prototype.description = function(definitions) {
  ...
  if (this.elements) {
    for (var k in this.elements) {
      _.assign(desc, this.elements[k].description(definitions));
    }
  } else {
...
WSDL.prototype._processNextInclude = function(includes, included, callback) {
  var self = this,
    include = includes.shift();

  if (!include || included[include.location])
    return callback();
  
  included[include.location] = true;
  ...
  open_wsdl(includePath, function(err, wsdl) {
    ...
    if(wsdl.definitions instanceof DefinitionsElement){
      ...
      var key = include.namespace || wsdl.definitions.$targetNamespace;
      self.definitions.schemas[key] = deepMerge(self.definitions.schemas[key], wsdl.definitions);
      includes = includes.concat(wsdl.definitions.includes);
    }
    self._processNextInclude(includes, included, function(err) {
      callback(err);
    });
  });
};
...
WSDL.prototype.processIncludes = function(callback) {
  ...
  this._processNextInclude(includes, included, callback);
};
